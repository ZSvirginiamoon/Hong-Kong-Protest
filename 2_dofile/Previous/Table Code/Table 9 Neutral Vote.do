...
esttab using "./Proposals/Final Tables/Table 9 Neutral Vote.tex", replace f ///
prehead(\begin{table}[H]\centering ///
\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi} ///
\caption{ "IV Regression Results on Neutral Votes and Vote Shares" } \begin{tabular}{l*{@M}{c}} \toprule[1.5pt] ) ///
b(3) se(3) star(* 0.10 ** 0.05 *** 0.01) ///
keep( log_Events_Count_1124 ) coeflabel(log_Events_Count_1124 "Protest Intensity" ) ///
label booktabs nonotes collabels(none) ///
mgroups("Votes, of Total Voters" "Total Vote Shares" , pattern(1 0 1 0 ) ///
