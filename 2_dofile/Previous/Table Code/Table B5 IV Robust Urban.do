...
*******************************************************************************
*               Table 8.5 IV Robustness controlling Urban Status              *
*******************************************************************************
eststo clear

eststo: reghdfe d2_投票率 density $Demo $Origin $Education $Employment $Income  , absorb(dc_class) vce(robust)
...
eststo: reghdfe d2_投票率 density $Demo $Origin $Education $Employment $Income  [aw=t_pop_2016], absorb(dc_class) vce(robust)
qui estadd local Demographics "Y"
qui estadd local Employment "Y"	
qui estadd local Income "Y"	
qui estadd local District "Y"
...
eststo: reghdfe d2_投票率 log_Events_Count_1124 density $Demo $Origin $Education $Employment $Income  , absorb(dc_class) vce(robust)
qui estadd local Demographics "Y"
qui estadd local Employment "Y"	
qui estadd local Income "Y"		
qui estadd local Weight "N"
...
eststo: reghdfe d2_投票率 log_Events_Count_1124 density $Demo $Origin $Education $Employment $Income  [aw=t_pop_2016], absorb(dc_class) vce(robust)
qui estadd local Demographics "Y"
qui estadd local Employment "Y"	
qui estadd local Income "Y"	
qui estadd local District "Y"
...
esttab using "./Proposals/Final Tables/Table B5 IV Robust Urban.tex", replace f ///
prehead(\begin{table}[H]\centering ///
\def\sym#1{\ifmmode^{#1}\else\(^{#1}\)\fi} ///
\caption{ "IV Regression: Regression Results Controlling Urban Suburban Status" } \begin{tabular}{l*{@M}{c}} \toprule[1.5pt] ) ///
b(3) se(3) star(* 0.10 ** 0.05 *** 0.01) ///
keep( log_Events_Count_1124 density ) coeflabel(log_Events_Count_1124 "Protest Intensity" density "Pop. Density" ) ///
